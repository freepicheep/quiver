...
pub fn install(project_dir: &Path, frozen: bool) -> Result<()> {
    ...
    install_resolved(&resolved_modules, &modules_dir, &lock_path, &nu_env_dir, &display_name)
}
...
mod tests {
    ...
    fn writes_activate_overlay_without_modules() {
        ...
        write_activate_overlay(&modules_dir, ".nu-env/modules", std::iter::empty::<&str>()).unwrap();
